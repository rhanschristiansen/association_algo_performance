...
def draw_boxes(img_names, boxes_dicts, class_names, model_size):
    ...
    for num, img_name, boxes_dict in zip(range(len(img_names)), img_names,
        ...
        cv2.imshow('img', cv2.cvtColor(np.array(img), cv2.COLOR_BGR2RGB))
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        # display(img)


def get_bboxes(boxes_dicts, class_names, model_size):
    """get detected boxes.
...
class Yolo_v3:
    ...
    def __init__(self, max_output_size, iou_threshold,
        ...
        self.class_names = load_class_names(os.path.join(THIS_DIR, 'coco.names'))
        self.n_classes = len(self.class_names)
        self.weights_path = os.path.join(THIS_DIR, 'yolov3.weights')
...
if __name__ == '__main__':
    ...
    img_names += glob.glob('./images/*.jpeg')
    ...
    model = Yolo_v3(max_output_size=max_output_size,
                    iou_threshold=iou_threshold,
                    confidence_threshold=confidence_threshold)
    model_size = _MODEL_SIZE
    inputs = tf.placeholder(tf.float32, [batch_size, 416, 416, 3])

    run_inference = model(inputs, training=False)
    ...
    with tf.Session() as sess:
        ...
        img_net = cv2.resize(img, (model_size[0], model_size[1]))
        batch = np.array([img_net])
        detection_result = sess.run(run_inference, feed_dict={inputs: batch})

        for idx, det in enumerate(detection_result):
            resize_factor = \
            ...
            for cls in range(len(class_names)):
                ...
                if np.size(boxes) != 0:
                    ...
                    for box in boxes:
                        ...
                        text = '{} {:.1f}%'.format(class_names[cls],
                                                   confidence * 100)
                        cv2.putText(img, text, (x1, y1), 1, 1, color)
